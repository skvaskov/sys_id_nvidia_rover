...
class UKF:
    def __init__(self, num_states, num_observations, process_noise, measurement_noise, initial_state, initial_inputs,initial_measurements,initial_covar, alpha, k, beta, iterate_function,observation_function):
        """
    ...
    def update(self,observations):
        ...
        :param observations: np array of indices (zero-indexed) of which states were measured, that is, which are being updated
    ...
    def get_inputs(self,index=-1):
        ...
        if index >= 0:
            ...
            return self.inputs
    def get_measurements(self,index=-1):
        """
        returns the current input (n_dim x 1), or a particular state variable (float)
        :param index: optional, if provided, the index of the returned variable
        :return:
    ...
    def reset(self, state, covar, meas='NONE'):
        """
