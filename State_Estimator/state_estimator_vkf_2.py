...
def yaw_jacobian(x_in):
    H=np.zeros([1,3])
    H[0][0]=-1
    return H

def update_encoder(data):
    global new_vel_measurement
    global observation_enc
    global last_vel_measurement_time
    global estimator_rate

    d_time = rospy.Time.now() - last_vel_measurement_time
    if d_time.to_sec()<1.0/estimator_rate/2.0:
        return
    mutex.acquire()
    new_vel_measurment=True
    observation_enc=data.velocity
    mutex.release()

def observe_vel(x_in):
    w=0.296/2.0
    r=0.05588
    vx=venc*r/2pi-w/2*w
    obs=np.zeros(1)
    obs[0]=-2.0*math.pi/r*x_in[1]-w*math.pi/r*x_in[0]
    return obs

def vel_jacobian(x_in):
    H=np.zeros([1,3])
    w=0.296/2.0
    r=0.05588
    H[0][0]=-w*math.pi/r
...
def general_update(data):
    ...
    if new_imu_measurement:
        state_estimator.update(observation_imu,observe_yaw,yaw_jacobian,0.01)
        new_imu_measurement=False
    if new_vel_measurement:
        state_estimator.update(observation_enc,observe_vel,vel_jacobian,0.01)
        new_vel_measurement=False
...
def definition():
    ...
    state_estimator = VKF(3, 2,M,X, np.zeros(2),P, math.sqrt(2), -1.0, 2.0, iterate_new_bicycle)
    estimator_rate=20.0
    mutex = Lock()
...
def main():
    ...
    sub4 = rospy.Subscriber('/joint_states',JointState,update_encoder)
