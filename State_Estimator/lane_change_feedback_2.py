...
def update_pose(data):
    ...
    global current_pose
    global pose_update_flag
    global lase_pose_update_time
    global feedback_rate
    if (rospy.Time.now()-last_pose_update_time).to_sec()<1.0/feedback_rate/2.0:
        return
    mutex.acquire()
    ...
    last_pose_update_time=rospy.Time.now()
    pose_update_flag=True
    mutex.release()

def update_twist(data):
    global current_twist
    global twist_update_flag
    global last_twist_update_time
    global feedback_rate
    if (rospy.Time.now()-last_twist_update_time).to_sec()<1.0/feedback_rate/2.0:
        return
    mutex.acquire()
    ...
    last_twist_update_time=rospy.Time.now()
    twist_update_flag=True
    mutex.release()
...
def follow_trajectory(data):
    ...
    #update values
    throttle_input=update_throttle(current_twist,twist_update_flag,vx_des)
    steering_input=update_steering(current_twist,twist_update_flag,w_des,vx_des)
    pose_update_flag=False
    twist_update_flag=False
...
def update_throttle(current_twist,tuf,vx_des):
...
def update_steering(current_twist,tuf,w_des,vx_des):
    ...
    if tuf:
        ...
        if vx_des>0:
            delta_guess=atan(l*w_des/vx_des)
            u1_guess=(delta_guess+0.008867066788855)/0.224314009055080
            u1=u1_guess
        else:
            u1=0.0395
