...
class VKF:
    def __init__(self, num_states, num_inputs,num_observations, input_noise, measurement_noise, initial_state, initial_inputs,initial_covar, alpha, k, beta, iterate_function,observation_function,observation_jacobian_function):
        """
        ...
        """
        self.n_dim = int(num_states)+int(num_inputs)
        ...
        self.H= observation_jacobian_function
    ...
    def __get_sigmas(self):
        ...
        tmp_mat = scipy.linalg.block_diag(self.p,self.m)

        spr_mat = scipy.linalg.cholesky(tmp_mat).T
        x_aug=np.concatenate((self.x,np.zeros(self.input_dim)))
        ret[0] = x_aug
    ...
    def predict(self, timestep):
        ...
        inputs=self.inputs
        #line (2.13)
        sigmas_out = np.array([self.iterate(x[range(self.state_dim)], timestep, inputs+x[range(self.state_dim,self.n_dim)]) for x in self.sigmas.T]).T
        #line (2.14)
